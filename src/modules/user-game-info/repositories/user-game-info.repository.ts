...
import { Currency, GameNumberPlayer, GameType, InGameCurrency } from "src/common"
...
export class UserGameInfoRepository {
    ...
    async incrementUserGameInfo(userId: string, incrementData: IncrementUserGameInfoDto): Promise<UserGameInfo | null> {
        ...
        const currencyFieldMapping: { [key in InGameCurrency]: { earn: string; spend: string } } = {
            [Currency.COIN]: { earn: "totalCoinEarned", spend: "totalCoinSpend" },
            [Currency.DIAMOND]: { earn: "totalDiamondEarned", spend: "totalDiamondSpend" },
            [Currency.LUTON]: { earn: "totalLutonEarned", spend: "totalLutonSpend" }
        }
